...
from Decryption.utils import yates_shuffle,generate_keys_ada

def ac_dec(key_data,block,user_key):
    '''
    RS pairs permutation within blocks
    Input: key_data: 密钥生成种子   block: 逆符号翻转后的DCT系数块
           user_key: 用户密钥
    Output: per_block: 进行块内逆RS对置换后的DCT系数块 
    '''
    num_rsv = []
    per_block = copy.deepcopy(block)
    block8_number = block.shape[2]
    for i in range(block8_number):
        rsv=[]
        temp = copy.deepcopy(block[:,:,i])
...
def ac_dec1(key_data,block,user_key):
    '''
    Global permutation of RS pairs
    Input: key_data: 密钥生成种子   block:逆块内置换后的DCT系数块
           user_key: 用户密钥
    Output: per_block:进行逆全局RS对置换后的DCT系数块 
    '''
    rsv = []
    num_rsv = []
    block8_number = block.shape[2]
    ...
    for i in range(len(run_type)):
        ...
        for j in range(run_num):
            per_temp[per_index[j],:] = temp_rsv[j,:]
    ...
    return per_block    #进行全局逆RS对置换后的DCT系数块